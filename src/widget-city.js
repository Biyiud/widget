...
async function renderAvoidedEmissions(containerId, defaultProvince, defaultCity) {
  ...
  if (!container) {
    console.error(`Container ${containerId} element not found`);
    return;
  }

  try {
    const total = await getCityData(defaultCity);
    const average = total.avoidedCO2 * 100;
    ...
    scoreValue.textContent = `${total.Co2emissions.toFixed(1)} `;
    ...
    averageContainer.classList.add(`biyiud-widget__average--${getScoreClass(average)}`);
    ...
    footer.classList.add("biyiud-widget__footer")
    ...
    selectCity.addEventListener("change", async (event) => {
      ...
      scoreValue.classList.add("biyiud-widget__score-value--loading");

      let total;
      try {
        total = await getCityData(value);
      } catch (err) {
        console.error(`[biyiud] Failed to load data for ${value}:`, err);
        return;
      } finally {
        selectProvince.disabled = false;
        selectCity.disabled = false;
        scoreValue.classList.remove("biyiud-widget__score-value--loading");
      }
      const averageResp = total.avoidedCO2 * 100;

      scoreValue.textContent = `${total.Co2emissions.toFixed(2)} `;
      city.textContent = ` ${event.target.options[event.target.selectedIndex].text}`;
      averageContainer.className = "";
      averageContainer.classList.add("biyiud-widget__average-container");
      averageContainer.classList.add(`biyiud-widget__average--${getScoreClass(averageResp)}`);
    ...
    console.error(`[biyiud] Failed to load data for ${defaultCity}:`, err);
    return;
  }
...
async function getCityData(ineCode) {
  if(!ineCode) return;
  if (_cacheBiyiud.has(ineCode)) return _cacheBiyiud.get(ineCode);
  
  const resp = await fetch(`https://api-config-prod-5praimar.nw.gateway.dev/cities/emissions/${ineCode}`);
  
  if (!resp.ok) throw new Error(`API error: ${resp.status}`);
    
  const data = await resp.json();
  _cacheBiyiud.set(ineCode, data.total);
  return data.total;
}

function getScoreClass(average) {
  if (average > 30) return 'good';
  if (average > 0) return 'regular';
